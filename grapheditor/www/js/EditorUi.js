...
EditorUi.prototype.updateActionStates = function()
{
  ...
  //:CHANGED:PANA:21/03/2018:Took out rotation
...
/**
 * ADDED:PELLEGRINI:24/03/2018
 * New function but copied from another one
 * Adds the label menu items to the given menu and parent.
 */
EditorUi.prototype.generatePython = function(forceDialog){
	if (!forceDialog && this.editor.filename != null){
		this.generate(this.editor.getOrCreateFilename());
	}
	else{
		var dlg = new FilenameDialog(this, this.editor.getOrCreatePythonFilename(), mxResources.get('generatePython'), mxUtils.bind(this, function(name){
			this.generate(name);
		}), null, mxUtils.bind(this, function(name){
			if (name != null && name.length > 0)
				return true;
 ...
 * New function but copied from another one
 * Saves the current graph under the given filename.
 * TODO : Status of the objects
 */
EditorUi.prototype.generate = function(name){
	if (name != null){
		if (this.editor.graph.isEditing()){
			this.editor.graph.stopEditing();
		}
		...
		try{
			if (Editor.useLocalStorage){
				...
				if (xml.length < MAX_REQUEST_SIZE){
					new mxXmlRequest(GENERATE_URL, 'filename=' + encodeURIComponent(name) +
						'&xml=' + encodeURIComponent(xml)).simulate(document, '_blank');
				}
				else{
					mxUtils.alert(mxResources.get('drawingTooLarge'));
...
EditorUi.prototype.createKeyHandler = function(editor)
{
	...
	if (!this.editor.chromeless || this.editor.editable)
	{
		...
		//MODIFIED:PELLEGRINI:24/03/2018:
		//changed shortcut Ctrl+G from group to generatePython
		//keyHandler.bindAction(71, true, 'group'); // Ctrl+G
		keyHandler.bindAction(71, true, 'generatePython'); // Ctrl+G
		keyHandler.bindAction(77, true, 'editData'); // Ctrl+M
		//MODIFIED:PELLEGRINI:24/03/2018:
		//changed shortcut Ctrl+Shift+G from grid to group layers
		//keyHandler.bindAction(71, true, 'grid', true); // Ctrl+Shift+G
		keyHandler.bindAction(71, true, 'group', true); // Ctrl+Shift+G
