...
Sidebar.prototype.init = function()
{
	...
	//:CHANGED:PELLEGRINI:13/02/2018:added the layer palette
	this.addLayerPalette(true);
};
 ...
 * :CHANGED:PELLEGRINI:28/07/2018:modified to search for partial words
 */
Sidebar.prototype.searchEntries = function(searchTerms, count, page, success, error){
	if (this.taglist != null && searchTerms != null){
		var searchTermList = searchTerms.toLowerCase().split(' ');
		var dict = new mxDictionary();
		var max = (page + 1) * count;
		var results = [];

		for (var i = 0; i < searchTermList.length; i++){
			if (searchTermList[i].length > 0){
				var tmpDict = new mxDictionary();
				var arr = [];
				var entry = this.taglist[searchTermList[i]];
				if (entry != null)
					arr = arr.concat(entry.entries);
				for(var testTag in this.taglist)
					if(testTag.includes(searchTermList[i])){
						var entries = this.taglist[testTag].entries;
						for(var elem in entries)
							if(!arr.includes(entries[elem]))
								arr.push(entries[elem]);
					}
				if(arr.length >= 1){
					results = [];
					for (var j = 0; j < arr.length; j++){
						...
						if ((i == 0) == (dict.get(entry) == null)){
							...
							results.push(entry);
							if (i == searchTermList.length - 1 && results.length == max){
								success(results.slice(page * count, max), max, true, searchTermList);
								return;
							}
						}
					}
		...
		success(results.slice(page * count, (page + 1) * count), len, false, searchTermList);
	}
	else{
		success([], null, null, searchTermList);
	}
};
...
/**
 * :ADDED:PANA:14/03/2018:
 *
 * Helper function used to add a layer to the sidebar.
 * This function was created to centralize the process of adding layers and
 * split the logic and rendering.
 *
 * Arguments :
 *    - Layer : The layer we want to add to the sidebar
 *
 * :CHANGED:PANA:28/03/2018:Uses layers directly now instead of name and search terms
 */
Sidebar.prototype.addKerasLayer = function(layerName) {
	var displayString = window.keras.layerToHTML(layerName);
	var search = 'keras ' + window.keras.getSearchTerm(layerName);
	return this.addEntry(search, mxUtils.bind(this, function(){
		return this.createVertexTemplate('rounded=0;whiteSpace=wrap;html=1;resizable=0;',
			120, 60, displayString, layerName, layerName, layerName, layerName);
	}));
}

/**
 * :ADDED:PELLEGRINI:16/02/2018:
 * Adds the Core layers palette to the sidebar.
 *
 * It does so by iterating over every layer by group and adds
 * them using the addKerasLayer function and the addPaletteFunctions function
 *
 * :CHANGED:PANA:14/03/2018:Used addKerasLayer instead
 * :CHANGED:PANA:28/03/2018:Sorts by group
 * :CHANGED:PELLEGRINI:02/04/2018:Heavily modified the algo
 */
Sidebar.prototype.addLayerPalette = function(expand){
	var context = this;
	groups = window.keras.getGroups();
	for(group in groups){
		var fns = [];
		for(layer in groups[group]){
			fns.push(this.addKerasLayer(groups[group][layer]));
		}
		context.addPaletteFunctions(group, group, (expand != null) ? expand : true, fns);
	}
};
...
Sidebar.prototype.createThumb = function(cells, width, height, parent, title, showLabel, showTitle, realWidth, realHeight){
	...
	if (this.sidebarTitles && title != null && showTitle != false){
		parent.style.height = (this.thumbHeight + this.sidebarTitleSize + 8) + 'px';
...
Sidebar.prototype.createItem = function(cells, title, showLabel, showTitle, width, height, allowCellsInserted){
	...
	//this.createThumb(cells, this.thumbWidth, this.thumbHeight, elt, title, showLabel, showTitle, width, height);//TODO remove?
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplate = function(style, width, height, value, title, showLabel, showTitle, allowCellsInserted, layer){
	var cells = [new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style)];
	cells[0].vertex = true;
	cells[0].setValue(value);
	var preferred = this.graph.getPreferredSizeForCell(cells[0]);
