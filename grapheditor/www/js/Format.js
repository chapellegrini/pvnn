...
Format.prototype.refresh = function()
{
	...
	else
	{
		...
		// :CHANGED:PANA:14/03/2018:Tweaked values and added label4 for KerasPanel
    	label.style.width = (containsLabel) ? '33.3%' : '25%';
	    ...
	    //:DELETED:PELLEGRINI:01/04/2018:The unused panels

	    // :ADDED:PANA:14/03/2018:Added Keras Panel
	    mxUtils.write(label4, 'Keras');
	    div.appendChild(label4);

	    var kerasPanel = div.cloneNode(false);
	    kerasPanel.style.display = 'none';
	    this.panels.push(new KerasFormatPanel(this, ui, kerasPanel));
	    this.container.appendChild(kerasPanel);

		//addClickHandler(label2, textPanel, idx++);
		//addClickHandler(label3, arrangePanel, idx++);
		addClickHandler(label4, kerasPanel, idx++);
	}
};
...
// :DELETED:PELLEGRINI:01/04/2018:Deleted unused panels
...
/*
 * :ADDED:PANA:30/03/2018: All functions past this comment
 * :CHANGED:PELLEGRINI:01/04/2018:
 */

KerasFormatPanel = function(format, editorUi, container)
{
  BaseFormatPanel.call(this, format, editorUi, container);
  this.init();
	this.container.appendChild(this.addView(this.createPanel()));
};
mxUtils.extend(KerasFormatPanel, BaseFormatPanel);

KerasFormatPanel.prototype.init = function() {
  var ui = this.editorUi;
  var editor = ui.editor;
  var graph = editor.graph;
  var ss = this.format.getSelectionState();
...
//CHANGED:PELLEGRINI:06/03/2018:Modified from the BaseFormatPanel's method
KerasFormatPanel.prototype.addUnitInput = function(unit, right, width, update, step, marginTop, disableFocus){
	container = document.createElement("div");
	container.style.height = '27px';//TODO CHANGE THIS FIXED SIZE FOR A PARAM
	marginTop = (marginTop != null) ? marginTop : 0;

	this.createStepper = function(input, update, height, disableFocus, defaultValue){
		height = (height != null) ? height : 8;
		...
		down.onclick = function(input){
			return (e) => {
				...
				if (!isNaN(val)){
					input.stepDown();
					input.onchange();//ADDED:03/06/2018:PELLEGRINI:Trigger the onchange method
				}
			}
		}(input);
		up.onclick = function(){
	...
	if(isNaN(step))
		step = 1;
	input.stepUp = function(input, step){
		return () => {input.value = +((Number(input.value) + step).toFixed(8));};
	}(input, step);
	input.stepDown = function(input, step){
		return () => {input.value = +((Number(input.value) - step).toFixed(8));};
	}(input, step);
	input.onchange = function(container){
		return () => {container.onchange();};
	}(container);

	var stepper = this.createStepper(input, update, null, disableFocus);
	stepper.style.marginTop = (marginTop - 2) + 'px';
	stepper.style.right = right + 'px';
	container.appendChild(stepper);

	container.getValue = function(input){
		return () => {return input.value;};
	}(input);
	container.setValue = function(input){
...
KerasFormatPanel.prototype.addNumericParam = function(div, param, layerInstance, update){// TODO BUG move to the left when there is too much elements in the panel for exemple Conv1D
	var inputUpdate = function(input, layerInstance, param){
		return (e) => {
			var n = Number(input.value);
			if(!isNaN(n)){
				if(!param.isFloat)
					n = input.value = Number(n.toFixed(0));
				if(param.min !== undefined)
					if(n <= param.min){
						if(param.leftClosed == true)
							n = input.value = param.min;
						else
							n = input.value = param.min + param.step;
					}
					if(param.max !== undefined)
						if(n >= param.max){
							if(param.rightClosed == true)
								n = input.value = param.max;
							else
								n = input.value = param.max - param.step;
						}
				layerInstance.setParam(param.name, n);
			}
			update();
		};
	}
	var input = this.addUnitInput('pt', 162, 44, inputUpdate(layerInstance, param), param.step);
	div.appendChild(input);
	if(layerInstance.getParam(param.name) != null)//TODO????
		input.input.value = layerInstance.getParam(param.name);
	input.onchange = inputUpdate(input.input, layerInstance, param);
}

KerasFormatPanel.prototype.addSelectParam = function(div, param, layerInstance, update){
	var select = document.createElement('select');
	for(optionName in param.list){
		var option = document.createElement('option');
		option.text = optionName;
		select.add(option);
	}
	select.paramName = param.name;
	if(layerInstance.getParam(param.name) != null)
		select.value = layerInstance.getParam(param.name);
	select.onchange = function(){
		layerInstance.setParam(this.paramName, this.value);
		update();
	};
	div.appendChild(select);
}

KerasFormatPanel.prototype.addTupleParam = function(div, param, layerInstance, update){
	var inputs = layerInstance.getParam(paramName);
	var addInputToTuple = function(layerInstance, paramName, position, addUnitInput){
		var input = addUnitInput('pt', 162, 44, update, param.step);//, inputUpdate, undefined, undefined, undefined, param);
		input.onchange = function(input, layerInstance, param, position){
			return () => {
				var n = Number(input.value);
				if(!isNaN(n)){
					if(!param.isFloat)
					...
					layerInstance.getParam(param.name)[position] = n;
				}
				update();
			};
		}(input.input, layerInstance, param, position);
		return input;
	};
	if(inputs == null){
		inputs = [];
		layerInstance.setParam(paramName, inputs);
		if(!isNaN(param.elementsNumber))
			for(var i = 0; i < param.elementsNumber; i++)
				inputs[i] = null;
	}
	var tupleDiv = document.createElement('div');
	if(isNaN(param.elementsNumber)){
		var buttonPlus = document.createElement('button');
		var buttonMinus = document.createElement('button');
		buttonPlus.innerHTML = '+';
		buttonPlus.onclick = function(inputs, tupleDiv, paramName, param, addUnitInput, buttonPlus, buttonMinus){
			return () => {
				tupleDiv.appendChild(addInputToTuple(layerInstance, paramName, inputs.length, addUnitInput));
				inputs.length++;
				if(!isNaN(param.elementsMaxNumber))
					if(inputs.length == param.elementsMaxNumber)
						buttonPlus.disabled = true;
				buttonMinus.disabled = false;
				update();
			};
		}(inputs, tupleDiv, paramName, param, this.addUnitInput, buttonPlus, buttonMinus);
		tupleDiv.appendChild(buttonPlus);
		buttonMinus.innerHTML = '-';
		buttonMinus.inputs = inputs;
		if(inputs.length == 0)
			buttonMinus.disabled = true;
		buttonMinus.onclick = function(inputs, tupleDiv, param, buttonPlus, buttonMinus){
			return () => {
				inputs.length--;
				if(inputs.length == 0)
					buttonMinus.disabled = true;
				if(!isNaN(param.elementsMinNumber))
				  if(inputs.length == param.elementsMinNumber)
						buttonMinus.disabled = true;
				buttonPlus.disabled = false;
				tupleDiv.childNodes[inputs.length + 2].remove();
				update();
			};
		}(inputs, tupleDiv, param, buttonPlus, buttonMinus);
		tupleDiv.appendChild(buttonMinus);
	}
	for(var i = 0; i < inputs.length; i++){
		var input = addInputToTuple(layerInstance, paramName, i, this.addUnitInput);
		tupleDiv.appendChild(input);
		input.input.value = layerInstance.getParam(paramName)[i];
	}
	if(!isNaN(param.elementsMinNumber))
		if(inputs.length == param.elementsMinNumber)
			buttonMinus.disabled = true;
	if(!isNaN(param.elementsMaxNumber))
		if(inputs.length == param.elementsMaxNumber)
			buttonPlus.disabled = true;
	div.appendChild(tupleDiv);
}

//CHANGED:PELLEGRINI:06/03/2018:Modified from the BaseFormatPanel's method
KerasFormatPanel.prototype.addParams = function(div, params, layerInstance, update){
  for(paramName in params){
		var param = params[paramName];
		div.appendChild(this.createTitle(paramName));
    if(param.getType() === 'numeric')
			this.addNumericParam(div, param, layerInstance, update);
		if(param.getType() === 'list')
			this.addSelectParam(div, param, layerInstance, update);
		if(param.getType() === 'tuple')
			this.addTupleParam(div, param, layerInstance, update);
	}
}

KerasFormatPanel.prototype.addView = function(div){
  var ui = this.editorUi;
  var editor = ui.editor;
  var graph = editor.graph;

  var ss = this.format.getSelectionState();
  var vertices = ss.vertices;
	//var vertices = this.editorUi.editor.graph.getSelectionCells();
	if(vertices.length != 1){
	  return div; // multiple selection not implemented
	}
  var selVertex = vertices[0];
	var value = selVertex.getValue();
	var layerInstance = new KerasLayerInstance(value);
	var selTitle = window.keras.extractTitle(value);
	if(! selTitle){
	  return div;
  }
	function updateParameters(){
		selVertex.setValue(layerInstance.writeHTML());
	}
	parameters = window.keras.getLayerParameters(selTitle);
  this.addParams(div, parameters, layerInstance, updateParameters);

  return div;
};

KerasFormatPanel.prototype.destroy = function()
{
   BaseFormatPanel.prototype.destroy.apply(this, arguments);
